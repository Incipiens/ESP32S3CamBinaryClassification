...
Once you've imported the project to PlatformIO, modify `main.cpp` and replace "WIFI NAME" and "WIFI PASSWORD" with your network details. Compile the application and upload it to your ESP32-S3. Results can be found by navigating to the IP address of your ESP32-S3 in your browser. Prediction results can be found on the /predict endpoint. Because the web server runs on a separate core to the inference and camera stack, the web server should always load with the previously cached image and results even when an inference task is being processed.
